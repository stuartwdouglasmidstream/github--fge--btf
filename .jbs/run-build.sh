...
export PROJECT_VERSION=1.2
export JAVA_HOME=/lib/jvm/java-1.8.0
export ENFORCE_VERSION=1.2
...
if [ -z "1.2" ]; then
  echo "Enforce version not set, recreating original tag 1.2"
  git tag -m 1.2 -a 1.2 || true
else
  echo "Creating tag 1.2 to match enforced version"
  git tag -m 1.2 -a 1.2 || true
fi
